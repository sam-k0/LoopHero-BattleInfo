...
namespace Config {
	...
	double ReadDoubleFromIni(const std::string& filename, const std::string& section, const std::string& keyname, double defaultValue = 0.0)
	{
		char buffer[256]; // Adjust the buffer size as needed
		GetPrivateProfileStringA(section.c_str(), keyname.c_str(), std::to_string(defaultValue).c_str(), buffer, sizeof(buffer), filename.c_str());

		// Convert the string buffer to a double
